...
unpd_raw <- read.csv("TooLarge/WPP2019_PopulationBySingleAgeSex_1950-2019.csv") ## FAILS UNTIL DOWNLOAD FROM:
# "https://population.un.org/wpp2019/Download/Files/1_Indicators%20(Standard)/CSV_FILES/WPP2019_PopulationBySingleAgeSex_1950-2019.csv"
...
life_table <- data.table(life_table_raw %>%
                           dplyr::mutate(AgeGrp = dplyr::case_when(AgeGrp == "4-Jan" ~ "1-4",
                                                                   AgeGrp == "9-May" ~ "5-9",
                                                                   AgeGrp == "14-Oct" ~ "10-14",
                                                                   TRUE ~ AgeGrp)))
...
world_bank$location_name <- revalue(world_bank$location_name,
  c("United States" = "United States of America",
    ...
    "Vietnam" = "Viet Nam"))
world_bank <- world_bank %>% dplyr::mutate(location_name = dplyr::case_when(location_name == "São Tomé and Principe" ~ "São Tomé and Príncipe",## New
                                                                            TRUE ~ location_name))

who <- rename(who_raw, c("Country" = "location_name"#"ï..Country" = "location_name"
                         ))
# REPLACED
...
#     "Swaziland" = "Eswatini"))
## Replaced with:
who <- who %>% dplyr::mutate(location_name = dplyr::case_when(location_name == "Iran, Islamic Republic of" ~ "Iran (Islamic Republic of)",
                                                              location_name == "Tanzania, United Republic of" ~ "United Republic of Tanzania",
                                                              location_name == "Antigua And Barbuda" ~ "Antigua and Barbuda",
                                                              location_name == "Bahamas" ~ "Bahamas, The",
                                                              location_name == "Cape Verde" ~ "Cabo Verde",
                                                              location_name == "Democratic Peoples Republic of Korea" ~ "North Korea",
                                                              location_name == "Korea, Republic of" ~ "South Korea",
                                                              location_name == "Lao People's Democratic Republic" ~ "Lao PDR",
                                                              location_name == "Macedonia, the former Yugoslav Republic of" ~ "North Macedonia",
                                                              location_name == "Micronesia, Federated States of" ~ "Micronesia",
                                                              location_name == "Moldova, Republic of" ~ "Moldova",
                                                              location_name == "Saint Kitts and Nevis" ~ "St. Kitts and Nevis",
                                                              location_name == "Saint Vincent and the Grenadines" ~ "St. Vincent and the Grenadines",
...
## New
who<- dplyr::arrange(who, location_name)
...
## New trialed content, deprecated
# munged_who_df <- who %>% dplyr::mutate(location_name = Country %>% purrr::map_chr(location_name, ~ c(.x,.y) %>% purrr::discard(is.na))) %>%
...
ppp_private_conversion <- ppp_private_conversion[, c("country", grep("X201", names(ppp_private_conversion
                                                                                   # UPDATED, ASSUMED TO REPLACE:
  #lcu_to_ppp_adj_usd
  ), value = TRUE))]
...
ppp_gdp_conversion <- ppp_gdp_conversion[, c("country", grep("X201", names(ppp_private_conversion
                                                                                   # UPDATED, ASSUMED TO REPLACE:
  #lcu_to_ppp_adj_usd
  ), value = TRUE))]
...
safety_exp_w_by_edu <- exp_w_by_edu
#detach(package:plyr)
exp_w_by_edu<- safety_exp_w_by_edu
## NEW 
exp_w_by_edu <- exp_w_by_edu %>% dplyr::mutate(exp_w_2019_USD = dplyr::case_when(variable=="no_schooling" & is.na(exp_w_2019_USD) ~ 50, #NEED TO UPDATE WITH REAL VALUE - PLACEHOLDER ONLY
                                               T ~ exp_w_2019_USD )) 
exp_w_by_edu <- exp_w_by_edu$country %>% unique() %>%
  purrr::map_dfr(~{
    country_1L_chr <- .x
    exp_w_by_edu$sex %>% unique() %>%
      purrr::map_dfr(~{
        sex_1L_chr <- .x
        filtered_tb <- dplyr::filter(exp_w_by_edu,country == country_1L_chr) %>% dplyr::filter(sex == sex_1L_chr) 
        filtered_tb %>%
          dplyr::mutate(exp_w_2019_USD = purrr::reduce(1:dplyr::n(),
                                                       .init = filtered_tb$exp_w_2019_USD, 
                                                       ~ #filtered_tb$return_to_1yr_school[.y]
                                                         {
                                                           if(.y==1){
                                                           .x[.y]<-.x[1]
                                                         }else{
                                                           .x[.y]<-(1+filtered_tb$return_to_1yr_school[.y])*.x[.y-1]
                                                         }
                                                       .x
                                                         }
                                                       ))
...
pr_edu <- rename(pr_edu_raw, c("ref_area.label" = "country",#"ï..ref_area.label" = "country", #REPLACED
                           "sex.label" = "sex",
...
# Replaced with:
pr_edu <- pr_edu %>% dplyr::mutate(pr_edu_level = n_edu_level / sum(n_edu_level, na.rm = TRUE))
...
### PICK UP HERE - NEED TO REPLACE DDPLYR
...
pr_emp_edu <- rename(pr_emp_edu_raw, c("ref_area.label" = "country",#"ï..ref_area.label" = "country",
                                   "sex.label" = "sex",
                                   "classif1.label" = "age_bands",
                                   "classif2.label" = "edu_level",
